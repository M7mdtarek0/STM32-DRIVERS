...
void GPIO_voidSetPinMode(Gpio_PortType Copy_u8Port, Gpio_PinType Copy_u8Pin, u8 Copy_u8Mode)
{
    if(Copy_u8Pin <= 15)  /* Check if the pin from 0 to 15 (all pins at the register) */
    {
        switch (Copy_u8Port)  /* Switch to find the port i want */
        {
        case GPIOA: /* Writing on the Port A*/
            ...
            break;
        case GPIOB: /* Writing on the Port B*/
            if(Copy_u8Pin <= 7) /* Check if the pin from 0 to 7 (Because it's the CRL (low)) */
            {
            ...
            break;
        case GPIOC: /* Writing on the Port C*/
            if(Copy_u8Pin <= 7) /* Check if the pin from 0 to 7 (Because it's the CRL (low)) */
            {
...
void GPIO_voidSetPinValue(Gpio_PortType Copy_u8Port, Gpio_PinType Copy_u8Pin, Gpio_ValueType Copy_u8Value)
{
    if(Copy_u8Pin <= 15)  /* Check if the pin from 0 to 15 (all pins at the register) */
    {
        switch (Copy_u8Port)  /* Switch to find the port i want */
        ...
        case GPIOA: /* Writing on the Port A*/
            ...
            else if(Copy_u8Pin == GPIO_LOW) /* Check if the pin value LOW */
            {
...
u8 GPIO_u8GetPinValue(Gpio_PortType Copy_u8Port, Gpio_PinType Copy_u8Pin)
{
